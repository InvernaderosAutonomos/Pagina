...
// Elementos del panel de Log In
const loginPanel = document.getElementById('panel-login');
const btnCerrarLoginPanel = document.getElementById('btn-cerrar-login');
...
function cambiarColores(colorPrincipal, colorOscuro, colorTexto, colorBotones) {
    ...
    document.querySelectorAll('.panel, .menu-configuracion, .panel-login').forEach(panel => {
...
// Resto del script (animaciones, sonidos, etc.) sigue igual...
 ...
 // Función para mostrar el panel de Log In
function mostrarLoginPanel() {
    clickSound.currentTime = 0;
    clickSound.play();

    // Aplicar fade-out al contenido principal
    mainContent.classList.add('fade-out');

    // Esperar a que termine la animación de fade-out
    setTimeout(() => {
        loginPanel.classList.add('mostrar'); // Mostrar el panel de Log In

        // Habilitar los botones del panel de Log In
        const botonesLogin = loginPanel.querySelectorAll('button');
        botonesLogin.forEach(boton => {
            boton.disabled = false;
        });
    }, 500); // 500ms = Duración de la animación de fade-out
}
 
// Función para ocultar el panel de Log In
function ocultarLoginPanel() {
    backSound.currentTime = 0;
    backSound.play();

    // Deshabilitar los botones del panel de Log In
    const botonesLogin = loginPanel.querySelectorAll('button');
    botonesLogin.forEach(boton => {
        boton.disabled = true;
    });

    // Aplicar fade-out al panel de Log In
    loginPanel.classList.remove('mostrar');

    // Esperar a que termine la animación de fade-out
    setTimeout(() => {
        mainContent.classList.remove('fade-out'); // Mostrar el contenido principal
    }, 500); // 500ms = Duración de la animación de fade-out
}
 
 // Evento de clic en el botón "Ingresar"
 btnIngresar.addEventListener('click', mostrarLoginPanel);
 
 // Evento de clic en el botón "Cerrar" del panel de Log In
 btnCerrarLoginPanel.addEventListener('click', ocultarLoginPanel);
